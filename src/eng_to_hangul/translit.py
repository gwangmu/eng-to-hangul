    ...
    def pack(self):
            ...
            if (tbl.is_whole_hangul_letter(self.cur_han)):
                ...
                if (self.top().initial.is_none()):
                    if (tbl.is_hangul_initial(self.cur_han)):
                        ...
                        self.top().set_vowel(self.cur_han, anno=self.has_anno)
                    else:
        ...
        for i, cur_hcl in enumerate(self.sent_hcl):
            ...
            if (isinstance(cur_hcl, hcl.HangulLetter) and isinstance(next_hcl, hcl.HangulLetter)):
                ...
                if ("no_self_consonants" in self.options and self.options["no_self_consonants"]):
            ...
            if (isinstance(cur_hcl, hcl.HangulLetter)):
                ...
                # Replace 'ㄹ,` to 'ㄹ'. ('ㄹ' as the final constant is already 'r')
                if (cur_hcl.final.value == 'ㄹ' and cur_hcl.final.has_anno()):
        ...
        if ("no_diacritical_mark" in self.options and self.options["no_diacritical_mark"]):
            for cur_hcl in self.sent_hcl:
...
def eng_to_ipa(sent_eng, options):
    ...
    if ("retrieve_all" in options and options["retrieve_all"]):
