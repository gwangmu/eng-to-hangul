...
class HangulConsonant:
    ...
    def __eq__(self, o):
        return (o and self.value == o.value and self.anno == o.anno)
...
class HangulLetter(Letter):
    def __init__(self, initial=None, vowel=None, final=None, whole=None, initial_anno=False, vowel_anno=False, final_anno=False):
        if (whole):
            whole_int = ord(whole)
        ...
        self.vowel = HangulVowel(vowel, anno=vowel_anno)
    ...
    def set_vowel(self, han, anno=False):
        self.vowel = HangulVowel(han, anno=anno)
    ...
    def __str__(self):
        ...
        if (self.initial.has_anno()):
            anno_front = "`"

        if (self.final.has_anno()):
            if (self.vowel.has_anno()):
                anno_back = ":"
            else:
                anno_back = ","
        else:
        ...
        if (not self.is_defined()):
            if (self.initial.is_none()):
                ...
                return (anno_front + self.initial.value + anno_back)
        else:
            return anno_front + self.fuse() + anno_back
