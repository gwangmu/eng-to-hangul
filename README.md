...
![image](https://github.com/user-attachments/assets/70e64f4f-59fd-49cd-9c7c-080f22cb376c)
...
Again, the diacritical marks were purposefully added to the closest Hangul vowels. The class suggests what needs attention.

| Class             	 | Sound 	| Vowel                                                                                   	| Example     | Explanation                    |
|---------             |-------	|-----------	                                                                              |-----------	|-------------                   |
| Tongue<br>(Iotation) | /ji/   | ![image](https://github.com/user-attachments/assets/9450a113-0174-489b-8256-de1410153da6) | ![image](https://github.com/user-attachments/assets/1df0672f-85dc-42fe-b2b0-65a28d2e93a8) (**Yi**eld) | **Iotate** 'ㅣ'. |

From my knowledge, the iotated 'ㅣ' (/i/) is the only missing vowel in the standard hangul. Technically, the hangul vowel 'ㅡ' (/ɯ/) also cannot be iotated in standard Hangul, but standard English doesn't have the /ɯ/ sound anyway, so we don't need to invent a notation to iotate 'ㅡ'. The diacritical mark for iotation (caron) was purposefully chosen so that it resembles the letter 'Y' when added over the character 'ㅣ'.

### Self-consonants

Self-consonants are simply **consonants without a vowel**. If you're unfamiliar with this concept, imagine pronouncing a letter with 'ㅡ'. If you can stop pronouncing at the constant (e.g., 'ㄴ', 'ㄹ', 'ㅁ', 'ㅅ', 'ㅇ', ...), don't pronounce 'ㅡ'. If not, pronounce 'ㅡ' very shortly.
...
Run the executable of your platform on the [Release](https://github.com/gwangmu/eng-to-hangul/releases) page.
...
한국어에 없는 자음을 표기하기 위헤 기존의 **가장 가까운** 자음 주변에 분음부호(예: ö 위의 점)를 붙입니다. 아래 표의 '종류'는 해당 자음에서 어떤 부분에 신경써야 하는 지를 의미합니다.
...
마찬가지로, 한국어에 없는 모음을 표기하기 위헤 기존의 **가장 가까운** 모음 주변에 분음부호를 붙입니다. 아래 표의 '종류'는 해당 모음에서 어떤 부분에 신경써야 하는 지를 의미합니다.

| 종류             	 | 발음  	| 모음                                                                                        	| 예            | 설명                    |
|---------             |-------	|-----------	                                                                              |-----------	|-------------                   |
| 혀<br>(이중모음화)    | /ji/   | ![image](https://github.com/user-attachments/assets/9450a113-0174-489b-8256-de1410153da6) | ![image](https://github.com/user-attachments/assets/1df0672f-85dc-42fe-b2b0-65a28d2e93a8) (**Yi**eld) | 'ㅣ'를 **이중모음화**. |

저자가 아는 바, 한글에 없지만 영어에는 존재하는 모음으로는 이중모음화된 'ㅣ'가 유일합니다. ('ㅡ' 역시 원칙적으로는 이중모음화 될 수 있지만 표준 영어에 'ㅡ' 발음이 없는 바, 해당 모음은 별도로 고려되지 않았습니다.) 이중모음화 부호(캐론, ˇ)는 'ㅣ'와 조합되었을 때 영문자 'Y'와 비슷하게 보이도록 의도적으로 선정되었습니다.

### 홀자음

홀자음은 **모음이 없는 자음**입니다. 모음 없는 자음에 익숙하지 않은 경우, 먼저 모음 'ㅡ'가 있다고 가정합니다 (예: 'ㄴ' -> '느', 'ㅂ' -> '브'). 만약 자음에서 발음을 끝낼 수 있으면 (예: '느') 'ㅡ'를 발음하지 않고, 아니면 (예: '브') 'ㅡ' 발음을 최대한 짧게 합니다.
...
[릴리즈](https://github.com/gwangmu/eng-to-hangul/releases) 페이지에서 해당 플랫폼에 맞는 바이너리를 다운받아 실행합니다.
